<!DOCTYPE html>
<html>
<head>
<title>Пищевые красители</title>
  <?php include('head.php'); ?>
  
</head>
<body>

	<div class="wrapper">
		<div class="content">
		<?php include('header.php'); ?>
        <div class="site__nav">
            <div class="container">
                <h2 class="site__nav-current">Жидкие красители</h2>
                <div class="breadcrumbs">
                    <span class="breadcrumbs__item"><a href="index.php">Главная</a></span>
                    <span class="breadcrumbs__item"><a href="accii.php">Каталог</a></span>
					<span class="breadcrumbs__item"><a href="accii.php">Пищевые красители</a></span>
                </div>
            </div>
        </div>

		<div class="catalog-category">
			<div class="container">
				<div class="catalog-category__items">
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category1.png" alt="">
							<p class="catalog-category__title">Сухие красители водорастворимые</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category2.png" alt="">
							<p class="catalog-category__title">Сухие красители жирорастворимые</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category3.png" alt="">
							<p class="catalog-category__title">Пищевые фломастеры и лаки</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category4.png" alt="">
							<p class="catalog-category__title">Гелевые красители</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category1.png" alt="">
							<p class="catalog-category__title">Гелевые красители</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category5.png" alt="">
							<p class="catalog-category__title">Кандурины</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category6.png" alt="">
							<p class="catalog-category__title">Пищевые блестки и глиттеры</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category4.png" alt="">
							<p class="catalog-category__title">Краски для аэрографа</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category7.png" alt="">
							<p class="catalog-category__title">Диоксид титана</p>
						</a>
						
					</div>
					<div class="catalog-category__item">
						<a href="">
							<img src="images/category2.png" alt="">
							<p class="catalog-category__title">Пыльца пищевая</p>
						</a>
						
					</div>
				</div>
			</div>
		</div>
		
		<section class="catalog-list">
			<div class="container">
				<div class="catalog-list__inner">
					<aside class="">
						<div class="catalog-menu">
							<div class="catalog-menu__top">
								<svg xmlns="http://www.w3.org/2000/svg"
									xmlns:xlink="http://www.w3.org/1999/xlink"
									width="17px" height="15px"  viewBox="0 0 17 15">
									<path fill-rule="evenodd"  fill="#54858f"
									d="M1.6405945,1.9999084 C1.6405945,1.9999084 15.3594055,1.9999084 15.3594055,1.9999084 C15.713211,1.9999084 15.999985,2.2867432 15.999985,2.640625 C15.999985,2.9945068 15.713211,3.281311 15.3594055,3.281311 C15.3594055,3.281311 1.6405945,3.281311 1.6405945,3.281311 C1.286789,3.281311 1.0000153,2.9945068 1.0000153,2.640625 C1.0000153,2.2867432 1.286789,1.9999084 1.6405945,1.9999084 ZM1.6428833,6.8437195 C1.6428833,6.8437195 15.357117,6.8437195 15.357117,6.8437195 C15.712158,6.8437195 15.999985,7.131653 15.999985,7.486664 C15.999985,7.486664 15.999985,7.5134583 15.999985,7.5134583 C15.999985,7.868469 15.712158,8.1562195 15.357117,8.1562195 C15.357117,8.1562195 1.6428833,8.1562195 1.6428833,8.1562195 C1.2878418,8.1562195 1.0000153,7.868469 1.0000153,7.5134583 C1.0000153,7.5134583 1.0000153,7.486664 1.0000153,7.486664 C1.0000153,7.131653 1.2878418,6.8437195 1.6428833,6.8437195 ZM1.6405945,11.718811 C1.6405945,11.718811 15.3594055,11.718811 15.3594055,11.718811 C15.713211,11.718811 15.999985,12.005615 15.999985,12.359314 C15.999985,12.713196 15.713211,13.0 15.3594055,13.0 C15.3594055,13.0 1.6405945,13.0 1.6405945,13.0 C1.286789,13.0 1.0000153,12.713196 1.0000153,12.359314 C1.0000153,12.005615 1.286789,11.718811 1.6405945,11.718811 Z"/>
								</svg>
								<span>КАТАЛОГ ТОВАРОВ</span>
							</div>
							<div class="catalog-menu__body">
								<ul class="catalog-menu__list">
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
												<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23.14 22.86"><defs><style></style></defs><path class="cls-1" d="M306.54,421.07v-3.55a2.56,2.56,0,0,0,1-1.21l0-2.35a1.9,1.9,0,0,0-1.4-1.77h-5.32a3.09,3.09,0,0,0-.71-2,3,3,0,0,0-4.07,0,3,3,0,0,0-.65,2H290a1.77,1.77,0,0,0-1.66,1.75v3l.2.15a5,5,0,0,0,1,.48v3.48h-3l0,.53c0,.37.5,3.36,8.78,4.13l1,2.85a4.17,4.17,0,0,0-2.69,3.05l-.09.58h9l-.09-.58a4.2,4.2,0,0,0-2.69-3.05l1-2.85c8.28-.77,8.75-3.76,8.78-4.13l0-.53Zm-9.72-10.26a1.62,1.62,0,0,1,1.28-.47,1.68,1.68,0,0,1,1.3.48,2.08,2.08,0,0,1,.45,1.36h-3.43A2.09,2.09,0,0,1,296.82,410.81Zm-7.5,3.16c0-.08.11-.8.67-.8h16a.88.88,0,0,1,.62.79V416a1.35,1.35,0,0,1-.62.66,10,10,0,0,1-2.63-.3,4.41,4.41,0,0,0-1.41-1.15l-.11-.06h-3.91a1.07,1.07,0,0,0-.84.34,1,1,0,0,0-.24.76v2.18s-.1.64-.4.64a1.18,1.18,0,0,1-.77-.35v-1.48c0-1.76-.75-1.93-2-2.07a2,2,0,0,0-1.86.75,6.67,6.67,0,0,1-.7.64,1,1,0,0,1-.57.17v-.07H290a2.78,2.78,0,0,1-.66-.28Zm1.17,3.8a1.88,1.88,0,0,0,1.18-.38,7.9,7.9,0,0,0,.8-.72c.39-.41.51-.53,1.07-.46,1.08.12,1.1.12,1.1,1.07v1.83l.1.13a2.23,2.23,0,0,0,1.67.87c1,0,1.33-1,1.4-1.58l0-2.33h3.75a3.53,3.53,0,0,1,.94.75c.1.19.36.66,3.06.75v3.38H290.49Zm10.83,13.44h-6.49a3.71,3.71,0,0,1,6.49,0Zm-2.49-2.86a5.71,5.71,0,0,0-.73-.05h-.05a5.58,5.58,0,0,0-.73.05l-.91-2.54c.53,0,1.07.06,1.66.08h0c.59,0,1.13,0,1.66-.08Zm-.76-3.46c-7.38-.21-9.6-1.93-10.26-2.82h20.53C307.67,423,305.45,424.68,298.07,424.89Z" transform="translate(-286.5 -409.34)"/></svg>
												<span>Мастика, марципан</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li> 



	

									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
												<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18.73 22.42"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M307.44,421.38l-.64-.77L305,422.13V409.56H291v10.06l-2.28-1.51v7.4l1.12.14v4.88a1.63,1.63,0,0,0,.37,1.1,1.15,1.15,0,0,0,.84.34H304.7l.17,0a1,1,0,0,0,.58-.18,1.44,1.44,0,0,0,.47-1.21v-4.54l1.36-4.55Zm-7,4.48-.4-.26h.72ZM304,421.6h-1.09v-2H304Zm-2.09,3h-2v-2h2Zm-2-3v-2h2v2Zm-1-5v2h-2v-2Zm-2-1v-2h2v2Zm3,1h2v2h-2Zm-1,3v2h-2v-2Zm5.09-1h-1.09v-2H304Zm0-3h-1.09v-2H304Zm-2.09,0h-2v-2h2Zm-2-3v-2h2v2Zm-1,0h-2v-2h2Zm-3,0h-2v-2h2Zm0,1v2h-2v-2Zm-3,2H292v-2h.88Zm0,1v2H292v-2Zm1,0h2v2h-2Zm2,3v2h-1.8v.06l-.2-.13V419.6Zm0,3v.25l-.37-.25Zm1,0h2v2h-.34l-1.66-1.09Zm6,0H304v.27h.11l-1.2,1Zm1.09-10h-1.09v-2H304Zm-11.09-2v2H292v-2Zm-.88,9h.88v1.27l-.88-.59Zm-2.28.37,3.16,2.09v.06H293l5.45,3.6-8.7-1.09Zm1.22,11a.65.65,0,0,1-.1-.38v-4.78l9.14,1.15V431H291C291,431,290.94,431,290.93,430.94Zm13.95.05H301v-2.87h4v2.49A3.06,3.06,0,0,1,304.88,431Zm.06-5.15v1.27h-4v-.39l4.93-4.06Z" transform="translate(-288.71 -409.56)"/></svg>
												<span>Шоколад, глазурь, какао</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.12 22.2"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M296.07,417.3v-5.53l0-.14c-.09-.32-.61-1.94-1.76-1.94H290.5a1.39,1.39,0,0,0-1,.39,2.51,2.51,0,0,0-.66,1.94v5.5a4.13,4.13,0,0,0-1.82,3.23v11.14h11.22V420.14A4.53,4.53,0,0,0,296.07,417.3ZM289.84,412c0-1.29.54-1.33.63-1.33h3.82c.25,0,.6.6.78,1.15v5.09a4,4,0,0,0-.86-.09h-4.12l-.2,0-.05,0Zm7.4,18.87H288V420.75c0-1.67,1.77-2.69,2.19-2.91h4a3.21,3.21,0,0,1,3,2.47Z" transform="translate(-287.02 -409.69)"/><path class="cls-1" d="M309.13,409.86h-9.19l2.78,17.25h.51v4.78h2.53v-4.78h.64Zm-7.2,6.08h5.23l-.34,2.14a3.7,3.7,0,0,0-2.28-.8,3.37,3.37,0,0,0-2.25.89Zm6-5.08-.65,4.08h-5.54l-.66-4.08Zm-5.47,8.56a2.62,2.62,0,0,1,2.05-1.14,2.84,2.84,0,0,1,2.09,1l-1.09,6.85h-2Zm2.27,11.47h-.53v-3.78h.53Z" transform="translate(-287.02 -409.69)"/><path class="cls-1" d="M292.75,421.93l-.36-.28-.31.35c-.25.28-2.41,2.79-2.28,4.65a2.66,2.66,0,0,0,2.83,2.54,2.9,2.9,0,0,0,2.53-2.58A7.31,7.31,0,0,0,292.75,421.93Zm-.12,6.26a1.67,1.67,0,0,1-1.83-1.62c-.08-1,1-2.65,1.71-3.53a6.76,6.76,0,0,1,1.65,3.57A2,2,0,0,1,292.63,428.19Z" transform="translate(-287.02 -409.69)"/></svg>
												<span>Пищевые красители</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.65 22.7"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M310.4,424.1a3.81,3.81,0,0,0-1.37-2.84l-.14-.11h-7.53v-6.44H291.05v13h5.67l.55,3.41H289a4.42,4.42,0,0,0,.59-2.25v-14.3a8.82,8.82,0,0,0-1.19-4.15H304a3.23,3.23,0,0,1-.51.84,5.07,5.07,0,0,0-.86,2.81v6.7h1v-6.69a4.11,4.11,0,0,1,.63-2.18,3.35,3.35,0,0,0,.86-2v-.5H286.49l.58.79a8.44,8.44,0,0,1,1.49,4.36v14.3c0,.59-.18,1.58-.56,1.82l-2.25,1.43h22.08l1.58-7.53h1Zm-1.23-1a3.56,3.56,0,0,1,.17.53H296.62c.13-.59.42-1.45,1-1.45h10.87A3,3,0,0,1,309.17,423.07Zm-17.12,3.65v-11h8.31v5.44h-2.7c-1.52,0-2.05,1.83-2.12,2.92l0,.53h.72l.34,2.12Zm15,4.41h-8.74l-1-6.53h11.15Z" transform="translate(-285.75 -409.43)"/></svg>
												<span>Упаковка, коробки</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.56 22"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M309.35,415.14h-5.71a5.22,5.22,0,0,0,1.94-1.19,3.1,3.1,0,0,0-.07-2.85c-.38-.64-1.25-1.37-3.18-1.26a4.75,4.75,0,0,0-3.95,2.87c-.95-1.25-2.93-3-5-2.88a3.21,3.21,0,0,0-2.81,1.3,2.63,2.63,0,0,0,0,2.49,3.51,3.51,0,0,0,2,1.52h-5.79v6.71h1.56v10H308v-10h1.38Zm-10.14,6.71v9h-2v-9Zm-2-1.28v-4.43h2v4.43Zm5.17-9.73c1.15-.07,1.91.19,2.27.78a2.14,2.14,0,0,1,.07,1.8,17.09,17.09,0,0,1-3.81,1.72l-2.1-.09a2.05,2.05,0,0,0,.16-1.21A4,4,0,0,1,302.39,410.84Zm-10.92,2.38a1.68,1.68,0,0,1,0-1.56,2.44,2.44,0,0,1,2-.83c2-.06,4.06,2.17,4.48,3a8.2,8.2,0,0,0-.27,1.16l0,.16h-.89C294.78,414.88,291.86,414.11,291.47,413.22Zm-3.68,7.63v-4.71h8.43v4.43h-7.87v.28Zm1.56,1h6.87v9h-6.87Zm17.62,9h-6.76v-9H307Zm1.38-10H308v-.28h-7.76v-4.43h8.14Z" transform="translate(-286.79 -409.83)"/></svg>
												<span>Кондитерские насадки и мешки</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.7 13.22"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M309.24,414.31a.5.5,0,0,0-.58,0l-1.15.73-.58-.64a.51.51,0,0,0-.71,0l-.91.83-1.3-.87a.51.51,0,0,0-.68.11l-.58.76-1.32-.87a.5.5,0,0,0-.39-.07.53.53,0,0,0-.32.23l-.43.71-1.27-.87a.5.5,0,0,0-.67.11l-.64.81-1.11-.89a.48.48,0,0,0-.37-.11.45.45,0,0,0-.33.19l-.65.81-1.1-.89a.48.48,0,0,0-.38-.11.51.51,0,0,0-.34.21l-.64.91-1.11-1a.53.53,0,0,0-.38-.12.49.49,0,0,0-.35.19l-.59.75-.93-.82a.51.51,0,0,0-.39-.12.5.5,0,0,0-.35.21l-.39.55-.75-.64a.51.51,0,0,0-.61,0,.5.5,0,0,0-.18.58l4.32,12.22a.51.51,0,0,0,.47.34h13a.51.51,0,0,0,.47-.33l4.37-12.23A.5.5,0,0,0,309.24,414.31Zm-3.56,1.84.65-.58-3.55,10.85h-.46l2.73-10.24h0A.5.5,0,0,0,305.68,416.15Zm-2.43,0,.59-.76.33.22-2.88,10.83h0l1.49-10.11A.49.49,0,0,0,303.25,416.13Zm-2.38,0,.44-.72.52.34-1.57,10.68H300l.66-10.1A.45.45,0,0,0,300.87,416.12Zm-2-.75.8.55-.68,10.5h-.37l.21-10.74h-.19Zm-1.06,1-.19,10h-.73V415.8l.61.49A.5.5,0,0,0,297.78,416.39Zm-1.32-.92h-.15l.06-.07Zm-1.07.93a.49.49,0,0,0,.33-.19l.14-.17v10.38h-.13L295,416.28h0A.54.54,0,0,0,295.39,416.4Zm-2.1,0,.65-.93.1.08H294l.77,10.91h-.22l-1.7-9.87.14,0A.51.51,0,0,0,293.29,416.35Zm-2.47-.18.59-.75.21.2,1.87,10.8h-.1l-2.94-10.06h0A.48.48,0,0,0,290.82,416.17Zm-2.54,0a.49.49,0,0,0,.52-.2l.36-.5,3.19,10.94h-.45Zm15.93,10.24h-.37l3.37-10.32a.51.51,0,0,0,.49,0l.26-.17Z" transform="translate(-286.73 -414.2)"/></svg>
												<span>Форма для шоколада</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23.34 23.16"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M303.64,418.86a.53.53,0,0,0-.46-.31.51.51,0,0,0-.47.32l-4.9,12.81a.5.5,0,0,0,.05.46.53.53,0,0,0,.42.21h10.14a.48.48,0,0,0,.41-.22.49.49,0,0,0,.05-.47Zm1.79,12.49v-3h-1v3H299l4.2-10.94,4.47,10.94Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M294.69,427.36c-.26-.46-.58-1-.76-1.42,3.77-.17,6.8-3.85,6.8-8.36s-3.17-8.38-7.07-8.38-7.07,3.76-7.07,8.38c0,4.29,2.75,7.83,6.26,8.31a9.67,9.67,0,0,0,1,2c.17.29.35.6.52.91a12.88,12.88,0,0,1,1,2.63l1-.25a14.31,14.31,0,0,0-1.13-2.86C295.05,428,294.87,427.66,294.69,427.36Zm-7.1-9.78c0-4.07,2.73-7.38,6.07-7.38s6.07,3.31,6.07,7.38S297,425,293.66,425,287.59,421.64,287.59,417.58Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M289.3,417.14l1,.17s.48-2.75,2-3.7l-.54-.84C289.86,414,289.33,417,289.3,417.14Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M303.43,411.58c0-.11-.08-1.1,1.78-1.39l-.15-1a3.25,3.25,0,0,0-2.24,1.15,1.94,1.94,0,0,0-.38,1.38Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M309.74,419.73a6.15,6.15,0,0,0-1.32-2.83l-.76.65a5.31,5.31,0,0,1,1.09,2.32Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M287.28,426.17l-.88.48a11.56,11.56,0,0,0,2.64,3.24l.59-.81A10.74,10.74,0,0,1,287.28,426.17Z" transform="translate(-286.4 -409.2)"/></svg>
												<span>Формы кондитерские</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23.14 22.86"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M306.54,421.07v-3.55a2.56,2.56,0,0,0,1-1.21l0-2.35a1.9,1.9,0,0,0-1.4-1.77h-5.32a3.09,3.09,0,0,0-.71-2,3,3,0,0,0-4.07,0,3,3,0,0,0-.65,2H290a1.77,1.77,0,0,0-1.66,1.75v3l.2.15a5,5,0,0,0,1,.48v3.48h-3l0,.53c0,.37.5,3.36,8.78,4.13l1,2.85a4.17,4.17,0,0,0-2.69,3.05l-.09.58h9l-.09-.58a4.2,4.2,0,0,0-2.69-3.05l1-2.85c8.28-.77,8.75-3.76,8.78-4.13l0-.53Zm-9.72-10.26a1.62,1.62,0,0,1,1.28-.47,1.68,1.68,0,0,1,1.3.48,2.08,2.08,0,0,1,.45,1.36h-3.43A2.09,2.09,0,0,1,296.82,410.81Zm-7.5,3.16c0-.08.11-.8.67-.8h16a.88.88,0,0,1,.62.79V416a1.35,1.35,0,0,1-.62.66,10,10,0,0,1-2.63-.3,4.41,4.41,0,0,0-1.41-1.15l-.11-.06h-3.91a1.07,1.07,0,0,0-.84.34,1,1,0,0,0-.24.76v2.18s-.1.64-.4.64a1.18,1.18,0,0,1-.77-.35v-1.48c0-1.76-.75-1.93-2-2.07a2,2,0,0,0-1.86.75,6.67,6.67,0,0,1-.7.64,1,1,0,0,1-.57.17v-.07H290a2.78,2.78,0,0,1-.66-.28Zm1.17,3.8a1.88,1.88,0,0,0,1.18-.38,7.9,7.9,0,0,0,.8-.72c.39-.41.51-.53,1.07-.46,1.08.12,1.1.12,1.1,1.07v1.83l.1.13a2.23,2.23,0,0,0,1.67.87c1,0,1.33-1,1.4-1.58l0-2.33h3.75a3.53,3.53,0,0,1,.94.75c.1.19.36.66,3.06.75v3.38H290.49Zm10.83,13.44h-6.49a3.71,3.71,0,0,1,6.49,0Zm-2.49-2.86a5.71,5.71,0,0,0-.73-.05h-.05a5.58,5.58,0,0,0-.73.05l-.91-2.54c.53,0,1.07.06,1.66.08h0c.59,0,1.13,0,1.66-.08Zm-.76-3.46c-7.38-.21-9.6-1.93-10.26-2.82h20.53C307.67,423,305.45,424.68,298.07,424.89Z" transform="translate(-286.5 -409.34)"/></svg>
												<span>Бумажные формы и капсулы</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23.34 23.16"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M303.64,418.86a.53.53,0,0,0-.46-.31.51.51,0,0,0-.47.32l-4.9,12.81a.5.5,0,0,0,.05.46.53.53,0,0,0,.42.21h10.14a.48.48,0,0,0,.41-.22.49.49,0,0,0,.05-.47Zm1.79,12.49v-3h-1v3H299l4.2-10.94,4.47,10.94Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M294.69,427.36c-.26-.46-.58-1-.76-1.42,3.77-.17,6.8-3.85,6.8-8.36s-3.17-8.38-7.07-8.38-7.07,3.76-7.07,8.38c0,4.29,2.75,7.83,6.26,8.31a9.67,9.67,0,0,0,1,2c.17.29.35.6.52.91a12.88,12.88,0,0,1,1,2.63l1-.25a14.31,14.31,0,0,0-1.13-2.86C295.05,428,294.87,427.66,294.69,427.36Zm-7.1-9.78c0-4.07,2.73-7.38,6.07-7.38s6.07,3.31,6.07,7.38S297,425,293.66,425,287.59,421.64,287.59,417.58Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M289.3,417.14l1,.17s.48-2.75,2-3.7l-.54-.84C289.86,414,289.33,417,289.3,417.14Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M303.43,411.58c0-.11-.08-1.1,1.78-1.39l-.15-1a3.25,3.25,0,0,0-2.24,1.15,1.94,1.94,0,0,0-.38,1.38Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M309.74,419.73a6.15,6.15,0,0,0-1.32-2.83l-.76.65a5.31,5.31,0,0,1,1.09,2.32Z" transform="translate(-286.4 -409.2)"/><path class="cls-1" d="M287.28,426.17l-.88.48a11.56,11.56,0,0,0,2.64,3.24l.59-.81A10.74,10.74,0,0,1,287.28,426.17Z" transform="translate(-286.4 -409.2)"/></svg>
												<span>Инструменты</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									<li class="catalog-menu__item">
										<div class="catalog-menu__show">
											<div class="catalog-menu__left">
											<svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23.14 22.86"><defs><style>.cls-1{fill:#3b395e;}</style></defs><path class="cls-1" d="M306.54,421.07v-3.55a2.56,2.56,0,0,0,1-1.21l0-2.35a1.9,1.9,0,0,0-1.4-1.77h-5.32a3.09,3.09,0,0,0-.71-2,3,3,0,0,0-4.07,0,3,3,0,0,0-.65,2H290a1.77,1.77,0,0,0-1.66,1.75v3l.2.15a5,5,0,0,0,1,.48v3.48h-3l0,.53c0,.37.5,3.36,8.78,4.13l1,2.85a4.17,4.17,0,0,0-2.69,3.05l-.09.58h9l-.09-.58a4.2,4.2,0,0,0-2.69-3.05l1-2.85c8.28-.77,8.75-3.76,8.78-4.13l0-.53Zm-9.72-10.26a1.62,1.62,0,0,1,1.28-.47,1.68,1.68,0,0,1,1.3.48,2.08,2.08,0,0,1,.45,1.36h-3.43A2.09,2.09,0,0,1,296.82,410.81Zm-7.5,3.16c0-.08.11-.8.67-.8h16a.88.88,0,0,1,.62.79V416a1.35,1.35,0,0,1-.62.66,10,10,0,0,1-2.63-.3,4.41,4.41,0,0,0-1.41-1.15l-.11-.06h-3.91a1.07,1.07,0,0,0-.84.34,1,1,0,0,0-.24.76v2.18s-.1.64-.4.64a1.18,1.18,0,0,1-.77-.35v-1.48c0-1.76-.75-1.93-2-2.07a2,2,0,0,0-1.86.75,6.67,6.67,0,0,1-.7.64,1,1,0,0,1-.57.17v-.07H290a2.78,2.78,0,0,1-.66-.28Zm1.17,3.8a1.88,1.88,0,0,0,1.18-.38,7.9,7.9,0,0,0,.8-.72c.39-.41.51-.53,1.07-.46,1.08.12,1.1.12,1.1,1.07v1.83l.1.13a2.23,2.23,0,0,0,1.67.87c1,0,1.33-1,1.4-1.58l0-2.33h3.75a3.53,3.53,0,0,1,.94.75c.1.19.36.66,3.06.75v3.38H290.49Zm10.83,13.44h-6.49a3.71,3.71,0,0,1,6.49,0Zm-2.49-2.86a5.71,5.71,0,0,0-.73-.05h-.05a5.58,5.58,0,0,0-.73.05l-.91-2.54c.53,0,1.07.06,1.66.08h0c.59,0,1.13,0,1.66-.08Zm-.76-3.46c-7.38-.21-9.6-1.93-10.26-2.82h20.53C307.67,423,305.45,424.68,298.07,424.89Z" transform="translate(-286.5 -409.34)"/></svg>
												<span>Декор</span>
											</div>
											<i class="fa fa-caret-right" aria-hidden="true"></i>
										</div>
										<ul class="catalog-menu__hidden">
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Жидкие красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители водорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Сухие красители жирорастворимые</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Пищевые фломастеры и лаки</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители</span>	
												</a>
											</li>
											<li>
												<a href="#">
													<i class="fa fa-caret-right" aria-hidden="true"></i>
													<span>Гелевые красители жирорастворимые</span>	
												</a>
											</li>
										</ul>
									</li>
									
								</ul>
							</div>
						</div>
						<div class="catalog-list__banner">
							БАННЕРНОЕ МЕСТО <br>
							ДЛЯ КАТАЛОГА
						</div>
						<div class="catalog-list__news">
							<p class="catalog-list__news-title">СВЕЖИЕ НОВОСТИ</p>
							<div class="catalog-list__news-items">
								<div class="ournews__item events">
									<p class="events__date">05.04.2021</p>
									<h3 class="events__title">Открытие нового магазина в Уссурийске</h3>
									<a href="" class="events__link">подробнее</a>
								</div>
								<div class="ournews__item events">
									<p class="events__date">03.04.2021</p>
									<h3 class="events__title">Поступление новых пищевых красок в наш ассортимент</h3>
									<a href="" class="events__link">подробнее</a>
								</div>
								<div class="ournews__item events">
									<p class="events__date">22.03.2021</p>
									<h3 class="events__title">График работы на майские праздники</h3>
									<a href="" class="events__link">подробнее</a>
								</div>
							</div>
							<a href="news.php" class="catalog-list__news-all">архив новостей</a>
						</div>

					</aside>

					<main>
						<div class="sale__nav">
                            <div class="sale__nav-sort">
                                <span>Сортировка:</span>
                                <select name="" id="" class="sale__sort">
                                    <option value="По алфавиту (возрастание);">По алфавиту (возрастание);</option>
                                    <option value="По алфавиту (убывание)">По алфавиту (убывание);</option>
                                </select>
                            </div>
                            <div class="sale__nav-sort--mob">
                                <span>На странице:</span>
                                <select name="" id="" class="sale__sort">
                                    <option value="">18</option>
                                    <option value="">36</option>
                                    <option value="">60</option>
                                </select>
                            </div>
                            <div class="sale__nav-nums">
                                <span>Товаров на странице:</span>
                                <span class="sale__nav-num sale__nav-num--active">18</span>
                                <span class="sale__nav-num">36</span>
                                <span class="sale__nav-num">60</span>
                            </div>
                        </div>

						<div class="catalog-list__param">
							<p class="catalog-list__param-title">
								<svg xmlns="http://www.w3.org/2000/svg" width="216.51" height="254.93" viewBox="0 0 216.51 254.93">
									<style type="text/css">
										.st0{fill:#4e72ab;}
									</style>
									<path class="st0" d="M194.9,20.36L22.01,20.36C23.09,22.31 23.79,23.77 24.65,25.13C47.42,61.34 70.28,97.48 92.87,133.8C94.92,137.1 96.17,141.45 96.22,145.34C96.52,170.88 96.38,196.44 96.4,221.99C96.4,223.35 96.6,224.71 96.77,226.76C103.47,221.12 109.46,215.91 115.64,210.92C119.01,208.19 120.42,204.91 120.39,200.58C120.28,181.5 120.27,162.41 120.46,143.33C120.49,140.68 121.28,137.67 122.68,135.44C145.63,98.76 168.74,62.18 191.8,25.57C192.75,24.07 193.62,22.52 194.9,20.36M85.45,254.93C78.55,250.97 76.36,245.01 76.44,237.14C76.75,208.09 76.64,179.04 76.45,149.99C76.44,147.04 75.52,143.72 73.96,141.23C52.1,106.19 30,71.3 8.13,36.27C5.17,31.53 2.5,26.43 0.79,21.14C-2.52,10.92 4.96,0.75 15.71,0.03C16.54,-0.02 17.37,0 18.2,0C78.3,0 138.39,0 198.49,0.01C207.66,0.01 214.59,4.7 216.21,12.85C216.88,16.22 216.39,20.21 215.18,23.46C213.41,28.23 210.79,32.74 208.07,37.08C186.33,71.8 164.44,106.43 142.76,141.19C141.21,143.68 140.31,146.99 140.27,149.93C140.05,167.53 140.17,185.13 140.16,202.72C140.15,211.14 137.19,218.54 130.89,223.97C118.62,234.54 105.94,244.63 93.42,254.93Z"/>
								</svg>
								<span>ПОДБОР ПО ПАРАМЕТРАМ</span>
							</p>
							<div class="catalog-list__param-items">
								<div class="catalog-list__param-item">
									<div class="catalog-list__param-show">
										<span>Бренд</span>
										<i class="fa fa-chevron-down" aria-hidden="true"></i>	
									</div>
									<div class="catalog-list__param-hidden">
										<div class="catalog-list__param-inner">
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adamex</span>
											</label>
											
										</div>	
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Camarelo</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adbor</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Alis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bebetto</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bobostello</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tako</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutes</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>		
										</div>
										
									</div>
								</div>
								<div class="catalog-list__param-item">
									<div class="catalog-list__param-show">
										<span>Назначение</span>
										<i class="fa fa-chevron-down" aria-hidden="true"></i>	
									</div>
									<div class="catalog-list__param-hidden">
										<div class="catalog-list__param-inner">
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adamex</span>
											</label>
											
										</div>	
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Camarelo</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adbor</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Alis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bebetto</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bobostello</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tako</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutes</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>		
										</div>
										
									</div>
								</div>
								<div class="catalog-list__param-item">
									<div class="catalog-list__param-show">
										<span>Материал</span>
										<i class="fa fa-chevron-down" aria-hidden="true"></i>	
									</div>
									<div class="catalog-list__param-hidden">
										<div class="catalog-list__param-inner">
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adamex</span>
											</label>
											
										</div>	
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Camarelo</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adbor</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Alis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bebetto</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bobostello</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tako</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutes</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>		
										</div>
										
									</div>
								</div>
								<div class="catalog-list__param-item">
									<div class="catalog-list__param-show">
										<span>Страна-изготовитель</span>
										<i class="fa fa-chevron-down" aria-hidden="true"></i>	
									</div>
									<div class="catalog-list__param-hidden">
										<div class="catalog-list__param-inner">
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adamex</span>
											</label>
											
										</div>	
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Camarelo</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Adbor</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Alis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bebetto</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Bobostello</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tako</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutes</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>
										<div class="catalog-list__param-variant">
											<label>
												<input  value="sdek"  class="check" type="checkbox">
												<span class="check-style"></span>
												<span>Tutis</span>
											</label>
											
										</div>		
										</div>
										
									</div>
								</div>
								<button class="catalog-list__param-reset">
									<span>+</span>
									<span>сбросить фильтр</span>
								</button>
							</div>
						</div>

						<div class="catalog-list__items">
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img class="catalog-list--sale" src="images/svg/sale10.svg" alt="">
                                    <img src="images/catalog11.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Голубая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price sale__item-price--new">
                                            <span>202</span>
                                             руб./шт.
                                        </p>
                                        <p class="catalog-list__item-price sale__item-price--old">
                                            <span>260</span>
                                             руб./шт.
                                        </p>
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img src="images/catalog12.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img src="images/catalog13.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/hit.png" alt="" class="catalog-list--hit">
                                    <img src="images/catalog14.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__zak">под заказ</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/new.png" alt="" class="catalog-list--new">
                                    <img src="images/catalog15.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/acciya.png" alt="" class="catalog-list--action">
                                    <img src="images/catalog16.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img class="catalog-list--sale" src="images/svg/sale10.svg" alt="">
                                    <img src="images/catalog11.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Голубая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price sale__item-price--new">
                                            <span>202</span>
                                             руб./шт.
                                        </p>
                                        <p class="catalog-list__item-price sale__item-price--old">
                                            <span>260</span>
                                             руб./шт.
                                        </p>
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img src="images/catalog12.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img src="images/catalog13.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/hit.png" alt="" class="catalog-list--hit">
                                    <img src="images/catalog14.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__zak">под заказ</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/new.png" alt="" class="catalog-list--new">
                                    <img src="images/catalog15.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/acciya.png" alt="" class="catalog-list--action">
                                    <img src="images/catalog16.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img class="catalog-list--sale" src="images/svg/sale10.svg" alt="">
                                    <img src="images/catalog11.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Голубая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price sale__item-price--new">
                                            <span>202</span>
                                             руб./шт.
                                        </p>
                                        <p class="catalog-list__item-price sale__item-price--old">
                                            <span>260</span>
                                             руб./шт.
                                        </p>
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img src="images/catalog12.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
                                    <img src="images/catalog13.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/hit.png" alt="" class="catalog-list--hit">
                                    <img src="images/catalog14.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__zak">под заказ</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/new.png" alt="" class="catalog-list--new">
                                    <img src="images/catalog15.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							<div class="catalog-list__item">
                                <div class="catalog-list__item-top">
									<img src="images/acciya.png" alt="" class="catalog-list--action">
                                    <img src="images/catalog16.jpg" alt="">
                                </div>
                                <p class="catalog-list__item-title">Жемчужная пыльца Желтая Kreda 5 гр</p>
                                <p class="catalog-list__item-code">Код товара: 003522;</p>
                                <div class="catalog-list__item-bottom">
                                    <div class="catalog-list__item-prices">
                                        <p class="catalog-list__item-price ">
                                            <span>185</span>
                                             руб./шт.
                                        </p>
                                        
                                    </div>
                                    <p class="catalog-list__has">в наличии</p>
                                </div>
                            </div>
							
							
						</div>
						
						<div class="pagination">
                                <div class="pagination__prev pagination__prev--disable">
                                    <i class="fa fa-chevron-left" aria-hidden="true"></i>
                                </div>
                                <div class="pagination__item pagination__item--active">1</div>
                                <div class="pagination__item">2</div>
                                <div class="pagination__item">3</div>
                                <div class="pagination__item">4</div>
                                <div class="pagination__item">5</div>
                                <div class="pagination__item">...</div>
                                <div class="pagination__item">10</div>
                                <div class="pagination__next ">
                                <i class="fa fa-chevron-right" aria-hidden="true"></i>
                                </div>
                        </div>
					</main>
				</div>
			</div>
		</section>

        
		
	
		<?php include('footer.php'); ?>
	</div>



	
  <?php include('modal.php'); ?>

</body>
</html>